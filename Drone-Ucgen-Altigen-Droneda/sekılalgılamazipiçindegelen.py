...
def detect_target_and_get_output(frame, model=None, search_for=None):
    ...
    # DÜZELTME 1: Renk aralıklarını ayarladık
    # Kırmızı renk için daha geniş HSV aralıkları
    lower_red1 = np.array([0, 80, 80])      # Saturation ve Value değerlerini azalttık
    upper_red1 = np.array([10, 255, 255])   
    lower_red2 = np.array([170, 80, 80])    # Saturation ve Value değerlerini azalttık
    upper_red2 = np.array([180, 255, 255])  

    # Mavi renk için de aynı şekilde
    lower_blue = np.array([100, 80, 80])    # Saturation ve Value değerlerini azalttık
        ...
        if area > 800:
            ...
            if M["m00"] != 0:
                ...
                if shape_type and (search_for is None or 
                    ...
                    # Görselleştirme - Kırmızı renge çiz
                    cv2.drawContours(vis, [contour], -1, (0, 0, 255), 4)  # Kırmızı
                    cv2.putText(vis, shape_name, (cx-80, cy-30),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)  # Kırmızı
                    cv2.circle(vis, (cx, cy), 5, (0, 0, 255), -1)  # Kırmızı
                    ...
                    if 0.75 <= aspect_ratio <= 1.25:  # Kare toleransı
                        ...
                        if extent > 0.5:  # Kare toleransı
                            ...
                            if search_for is None or search_for == "hexagon":
                                ...
                                # Görselleştirme - Mavi renge çiz
                                cv2.drawContours(vis, [contour], -1, (255, 0, 0), 4)  # Mavi
                                cv2.putText(vis, shape_name, (cx-80, cy-30),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)  # Mavi
                                cv2.circle(vis, (cx, cy), 5, (255, 0, 0), -1)  # Mavi
...
def test_detection():
    ...
    # USB kamera yerine Picamera2 kullan
    cap = cv2.VideoCapture(2)
    
    
    print("Test modu - q ile çıkış")
    print("EN İYİ algılama: Kırmızı üçgen (tracking3) + Kırmızı kare + Mavi altigen (tracking4)")
    print("DİKKAT: Renk düzeltmesi yapıldı!")
    ...
    while True:
        try:
            # Picamera2'den frame al
            ret, frame = cap.read()            
            # DÜZELTME 2: Direkt BGR formatında kullan (dönüşüm YOK!)
            # Picamera2 RGB formatında veri veriyor, OpenCV BGR bekliyor
            # Bu yüzden dönüşümü tersine çevireceğiz
            frame_bgr = frame.copy()
            
            vis, target_type, center = detect_target_and_get_output(frame_bgr)
            stream_server.update_frame(vis)

            frame_count += 1
            
            # Her 30 frame'de bir debug bilgisi
            if frame_count % 30 == 0:
                print(f"Frame {frame_count}: Target: {target_type}, Center: {center}")
            
            try:
                cv2.imshow('Duzeltilmis Renk Algilama', vis)
            except:
                print(f"GUI yok - Target: {target_type}, Center: {center}")
